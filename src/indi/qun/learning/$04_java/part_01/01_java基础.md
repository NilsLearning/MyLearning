...
### 11.如何保证集合是线程安全的? ConcurrentHashMap如何实现高效地线程安全？
- 早期的concurrentHashMap实现方式：
...
- 1.8的concurrentHashMap实现方式：
  - 1.在put数据时，首先判断容器是否为空，为空则利用CAS进行初始化。不为空则判断存放的桶是否为空，为空则利用CAS设置新节点。不为空则使用Synchronized对该桶加锁，遍历桶中的链表（或树），替换或新增节点到桶中
---
### 12.JAVA提供了哪些IO方式？NIO如何实现多路复用？
Java的IO发展可分为三个阶段，分别为以下三个阶段：
- 1.早期的IO阶段，比如File抽象，交互方式是同步，阻塞的方式
- 2.在1.4时，引入了NIO框架(java.nio包)，提供了Channel，Buffer等新的抽象，可以构建多路复用的，同步非阻塞IO程序
- 3.在1.7时，引入了异步非阻塞的IO方式（Asynchronous IO）,有些人称它为AIO，异步IO操作基于事件和回调机制，可以简单理解为,应用操作直接返回，而不会阻塞在那里，当后台处理完成，操作系统会通知相应线程进行后续工作。
NIO能解决什么问题：
例如：服务器同时服务多个客户端请求
传统的IO方式架构图：
![img.png](传统IO.png)
...
NIO方式：
![img.png](NIO.png)
其利用了单线程轮询事件的机制，通过高效地定位就绪的 Channel，来决定做什么，仅仅 select 阶段是阻塞的，可以有效避免大量客户端连接时，频繁线程切换带来的问题，应用的扩展能力有了非常大的提高
---
### 13.Java有几种文件拷贝方式？
传统IO方式
NIO方式
